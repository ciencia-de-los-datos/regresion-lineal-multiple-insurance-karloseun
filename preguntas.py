...
def pregunta_01():
    ...
    # ____.____(____) solved in last step
...
def pregunta_03():
    ...
    """
    # Verifying step 1: oneHotEncoder in make_column_transfor 
    mct = make_column_transformer(
        # ( OneHotEncoder(handle_unknown='ignore'), ['sex','smoker','region']), --> Ok!
        ( OneHotEncoder(handle_unknown='ignore'), make_column_selector(dtype_include='object') ),
        remainder="passthrough", 
    ).fit_transform(X_train, y=None) 
    print (mct[:5,:])
    print (mct.ndim)
    print (mct.shape)
    print (mct.size)
    ...
    param_grid = {
        ...
        # "linearRegression__fit_intercept": np.linspace( start=1, stop=11, num=11 ).astype(int),
    }

    # print (param_grid.get('linearRegression'))
    # print (np.linspace( start=1, stop=11, num=11 ).astype(int))
